app.controller('UserOperationCtrl', function ($scope, $http, $window, $fancyModal, $timeout) {
    ...
    $scope.Cls_User = { User:{ID:"",NAME:"",Username:"",Password:"",EMAIL:"",ROLE:"",COMPUTERNAME:"",COMPUTERUSER: "", PROGRAMUSER: "", PROGRAMUSER: "", INDATE: "", STOPEMP: "", PROGRAMUSERID: "" }
        , Cls_Views: null
    };
    var Search;
    var page = 0;
    ...
    $scope.switchCase = function () {
        var UserID = document.URL.split("/")[5];
        if (UserID == null) {
            $scope.SaveBtn = 1;
            $scope.EditBtn = null;
            ...
            $http.get("/Users/loadUser/" + UserID)
                .success(function (response) {
                    if (response != null) {
                        ...
                        for (var i = 0; i < response.Cls_Views.length; i++) {
                            $scope.dataAuthentication.push(response.Cls_Views[i]);
                        }
                         
                        $scope.Cls_User.User = response.User;// {User: , Cls_Views: response.Cls_Views};
                        $scope.Cls_User.Cls_Views = $scope.dataAuthentication;
                        console.log($scope.dataAuthentication);
    ...
    $scope.AddAuthentication = function (data, index) {
        ...
        if ($scope.dataAuthentication == null) {
            ...
            //$scope.Cls_Views = new Array();
        }
        else {
            for (var i = 0; i < $scope.dataAuthentication.length; i++) {
                if ($scope.dataAuthentication[i].View.ID == data.View.ID) {
                    checkExist = true;
                }
            }
            if (checkExist == false) {
    ...
    $scope.SaveUser = function () {
        $scope.Cls_User.Cls_Views = new Array();
        $scope.Cls_User.Cls_Views= $scope.dataAuthentication;
    ...
    $scope.Form_Validation = function (Cls_User) {
        ...
        if (Cls_User.Cls_Views == null || Cls_User.Cls_Views.length == 0) {
                 ...
                 .success(function (response) {
                     if (response.ErrorName.indexOf("صلاحية") > -1 || response.ErrorName.indexOf("المسموح") > -1) {
                         ...
                         if (response.ErrorName.indexOf("بنجاح") > -1) {
                             $timeout(function () {
                                 $window.location.href = '/users/operation/' + response.UserID;
                             }, 1000);
                         }
                     }
                 });
