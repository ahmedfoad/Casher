app.controller("ProductListCtrl", function ($scope, $http, $window, $timeout, $uibModal) {
    $scope.Intaize_product = function () {
        var Product = { ID: 0, Name: "", Barcode: "", Company_Id: "", Company_Name: "", Price_Unit: "", Taxes: "", Carton_Barcode: "", Carton_ProCount: "", Date_Expiration: "", Price_Mowrid: "" };
             ...
             alert("عفوا هذا الصنف تم حفظه من قبل براجاء اختيار صنف جديد");
              ...
              if (index == -1) {
                  ...
                  if ($scope.Cls_Product[index].Barcode == "") {
                      ...
                      $scope.Cls_Product[index].Carton_Barcode = parseFloat(word);
                  }
                 // var Product = { ID: 0, Name: "", Barcode: "", Company_Id: "", Company_Name: "", Price_Unit: "", Taxes: "", Carton_Barcode: "", Carton_ProCount: "", Date_Expiration: "", Price_Mowrid: "" };
   ...
   // $scope.getAllRecords();
    ...
    $scope.clicktr = function (row) {
        var index = parseFloat($scope.selected_Index);
        $scope.Cls_Product[index].Company_Id = row.ID;
        $scope.Cls_Product[index].Company_Name = row.Name;
    ...
    $scope.FormValidation = function () {
        ...
        for (var i = 0; i < len; i++) {
            $scope.Cls_Product[i].Date_Expiration = $("input[name='Date_Expiration[" + i + "]']").val();
        }
        for (var i = 0; i < len; i++) {
            if (len = 1
                ||
